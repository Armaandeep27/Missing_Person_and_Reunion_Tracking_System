...
const cfg = {
  ...
  support:{ endpoint:'/api/support-programs', list:'support-programs', id:'id', title:'Support Program', writeRoles:['admin','sponsor'], fields:[['personId','Case','person'],['sponsorName','Sponsor Name','text'],['sponsorPhone','Sponsor Phone','text'],['supportType','Support Type','select',['Sponsorship','Scholarship','Medical Aid','Shelter Aid','Food Support','Other']],['amount','Amount','number'],['startDate','Start Date','date'],['status','Status','select',['Planned','Active','Completed','Paused']],['notes','Notes','textarea']], columns:['case_no','full_name','sponsor_name','support_type','amount','start_date','status'] },
...
const canWrite = cfg?.writeRoles?.includes(currentUser?.role);
...
function buildForm(row={}){
  ...
  form.innerHTML = cfg.fields.map(f=>{
    ...
    if(type==='select') return '<div class="field"><label>'+label+'</label><select name="'+name+'">'+extra.map(o=>'<option '+(o==value?'selected':'')+'>'+o+'</option>').join('')+'</select></div>';
    if(type==='person'||type==='agency') return '<div class="field"><label>'+label+'</label><select name="'+name+'">'+options(type)+'</select></div>';
    if(type==='textarea') return '<div class="field full"><label>'+label+'</label><textarea name="'+name+'">'+esc(value)+'</textarea></div>';
    return '<div class="field"><label>'+label+'</label><input name="'+name+'" type="'+type+'" value="'+esc(value)+'" '+(extra?'placeholder="'+esc(extra)+'"':'')+'></div>';
...
function render(){
  ...
  head.innerHTML='<tr>'+cfg.columns.map(c=>'<th>'+esc(c.replaceAll('_',' '))+'</th>').join('')+(canWrite?'<th>Actions</th>':'')+'</tr>';
  rowsEl.innerHTML=filtered.map(r=>'<tr>'+cfg.columns.map(c=>'<td>'+(/status|confidence|outcome/.test(c)?badge(r[c]):esc(r[c]))+'</td>').join('')+(canWrite?'<td class="action-cell"><button class="icon-button" onclick="editRecord('+r[cfg.id]+')"><i class="fa-solid fa-pen"></i></button><button class="icon-button danger" onclick="deleteRecord('+r[cfg.id]+')"><i class="fa-solid fa-trash"></i></button></td>':'')+'</tr>').join('') || '<tr><td colspan="20">No records found.</td></tr>';
}
async function load(){
  try{
...
window.editRecord=id=>{ if(!canWrite) return; editingId=id; buildForm(records.find(r=>r[cfg.id]===id)); window.scrollTo({top:0,behavior:'smooth'}); };
window.deleteRecord=async id=>{ if(!canWrite) return; if(!confirm('Delete this record?')) return; const res=await fetch(cfg.endpoint+'/'+id,{method:'DELETE',headers:authHeaders()}); const out=await res.json(); if(!out.ok) alert(out.message); await load(); };
